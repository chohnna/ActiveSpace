...
for i in range(0, w -1):
    active_sizes.append(2*(i+1))

    occ_list = list(range(w + i +1, num_orbitals))
    act_list = list(range(w - i-1, w+i+1))
    vir_list = list(range(0,w -i-1))
...
plt.subplot(1, 2, 1)
plt.plot(active_sizes, elists, marker='.', linestyle='-', label='CIS_act')
...
plt.subplot(1, 2, 2)
plt.plot(active_sizes, elistt, marker='.', linestyle='-', label='CIS_act')
